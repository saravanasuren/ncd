  ...
  return db.withTx(async (tx) => {
    ...
    if (edits && req.entity_type === 'applications' && req.entity_id) {
      ...
      if (sets.length) {
        ...
        // said 28 from 01-08). The stored schedule was right because it is
        // materialised AFTER approval from the corrected application date —
        // which is also why nothing needs rebuilding here.
        //
        ...
        if ('date_money_received' in edits) {
          ...
          if (d !== null) {
            await tx.query(
              `UPDATE application_lines SET date_money_received = $1
                WHERE application_id = $2
                  AND (SELECT count(*) FROM application_lines x WHERE x.application_id = $2) = 1`,
              [d, Number(req.entity_id)]);
          }
        }
        await writeAudit(tx, {
