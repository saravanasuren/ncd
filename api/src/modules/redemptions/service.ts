...
async function createRequest(
  ...
  // TDS on the broken interest (same rule the interest run uses). The interest
  // is RECORDED, not added to the settlement: a redemption pays the redemption
  // amount (owner 2026-07-24), and 96 of 97 live redemptions already read
  // net_payment = principal − penalty. It still gets its own BrokenInterest
  // disbursement row at approval, carrying this TDS.
  let brokenTds = 0;
  ...
  const netPayment = calc.netPayment;   // principal − penalty; interest excluded
...
export async function adjustPrematurePenalty(db: Db, actor: AuthUser, redemptionId: number, input: { new_penalty: number; reason: string }) {
  ...
  return db.withTx(async (tx) => {
    ...
    // net = principal − penalty. The accrued interest is settled on its own
    // BrokenInterest row and never rode in net_payment, so nothing to preserve.
...
registerOnFinalApprove('premature_redemption', async (tx, req) => {
  ...
  if (lineId) {
    // net_payment IS the principal redemption (principal − penalty). The
    // accrued interest is a SEPARATE row: 'Premature' (principal, no TDS) and,
    // when there is accrued interest, 'BrokenInterest' with its own TDS — so
    // TDS reporting captures the interest tax. The TDS is read from the stored
    // column; deriving it from net_payment would book the whole gross as tax
    // now that net_payment no longer carries the interest.
    const principalNet = round2(Number(red.principal) - Number(red.penalty));
    const brokenGross = Number(red.broken_interest);
    const brokenTds = round2(Number(red.broken_tds ?? 0));
