...
beforeAll(async () => {
  ...
  const app = await a.post('/api/applications', { customer_id: cust.json.id, series_id: seriesId, scheme_id: schemeId, amount: 1500000 });
  appId = Number(app.json.id);
  await a.post(`/api/applications/${appId}/confirm-collection`, { amount_received: 1500000, date_money_received: '2026-07-10', method: 'NEFT' });
