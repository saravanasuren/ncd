...
export const SETTINGS_CATALOG: SettingDef[] = [
  ...
  {
    key: 'reports.transactions_email_enabled',
    group: 'Reports',
    label: 'Email the transaction register daily',
    description: 'When on, the Transactions sheet (every addition and deletion, the same tab as in the NCD book) is emailed once a day to the recipients below. Off by default so turning it on is a deliberate act.',
    type: 'boolean',
    default: false,
    editableBy: 'super_admin',
  },
  {
    key: 'reports.transactions_email_time',
    group: 'Reports',
    label: 'Transaction register send time (IST, HH:MM)',
    description: '24-hour IST time the daily transaction register is emailed, e.g. 19:00. It fires within ~15 minutes of this time. Invalid values fall back to 19:00.',
    type: 'string',
    default: '19:00',
    editableBy: 'super_admin',
  },
  {
    key: 'reports.transactions_email_recipients',
    group: 'Reports',
    label: 'Transaction register recipients',
    description: 'Email addresses that receive the daily transaction register. Nothing is sent while this is empty — there is no role fallback, because this attachment carries the whole book.',
    type: 'list',
    default: [],
    editableBy: 'super_admin',
  },
