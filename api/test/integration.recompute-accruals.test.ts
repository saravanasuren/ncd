...
describe('recompute accruals for a late referrer', () => {
  it('creates the missing referrer accrual, idempotently', async () => {
    const a = await admin();
    // Enrol + fund + activate WITHOUT a referrer (a "Direct" app).
    ...
    const app = await a.post('/api/applications', { customer_id: cust.json.id, series_id: seriesId, scheme_id: schemeId, amount: 300000 });
    const appId = Number(app.json.id);
    await a.post(`/api/applications/${appId}/confirm-collection`, { amount_received: 300000, date_money_received: '2026-07-10', method: 'NEFT' });
